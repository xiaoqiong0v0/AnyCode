...
def _validate_ref_fields(calc_fields):
    ...
    for f in calc_fields:
        deeps[f.name] += 0  # 避免没初始化
        l = []
        for b in f._ref_fields_names:
            deeps[b] += 1
            if b in names:
                l.append(b)
        links[f.name] = l
...
def _init_fn(fields, ref_fields, std_fields, kw_only_fields, frozen, has_post_init,
    ...
    for f in ref_fields:
        for ff in f._follow_fields or ():
            # 初始化调用更新所有值
            body_lines.append(f'{self_name}.{_fn_name_update(ff.name)}()')
...
def _update_fn(field, globals):
    ...
    body_lines = [f'self._{field.name}={default_name}({args})']
...
def _process_class(cls, init, repr, eq, order, unsafe_hash, frozen,
    ...
    # 检查回环依赖
    links, deeps = _validate_ref_fields(all_calc_fields)
    having_ref_fields = [fields[k] for k in deeps.keys()]
    # 按深度倒序
    sorted_having_ref_fields = sorted(having_ref_fields, key=lambda f: deeps[f.name])
