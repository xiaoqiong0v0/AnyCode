...
_FIELD_CALC = _FIELD_BASE('_FIELD_CALC')
...
class Field:
    ...
    def __init__(self, default, default_factory, init, repr, hash, compare,
        ...
        self._ref_fields_names = None
        self._ref_fields = None
        self._follow_fields = None

    def _add_follow_field(self, f):
        if self._follow_fields is None:
            self._follow_fields = [f]
        else:
            self._follow_fields.append(f)
    ...
    def __repr__(self):
        return ('Field('
                ...
                f'_ref_fields_names={self._ref_fields_names!r},'
                f'_ref_fields={self._ref_fields!r},'
                f'_follow_fields={self._follow_fields!r}'
                ')')
...
def field(*, default=MISSING, default_factory=MISSING, init=True, repr=True,
    ...
    如果 default_factory 有参数, 哪么将变成只读属性且不会出现在init参数中
    It will be read only and not include by init, if default_factory have args.
    """
...
def _validate_ref_fields(calc_fields):
    # check loop references
    names = [f.name for f in calc_fields]
    links = {}
    for f in calc_fields:
        links[f.name] = [b for b in f._ref_fields_names if b in names]
    cycle = _find_cycle(links)
    if cycle:
        raise ValueError(f"listen fields have a cycle: {" -> ".join(cycle)}")
    return links


def _deep_ref(fields):
    deeps = defaultdict(int)

    def _inner_deep_ref(field):
        if not isinstance(field, Field):
            return
        deeps[f.name] += 0
        if field._field_type != _FIELD_CALC:
            return
        for b in field._ref_fields_names or ():
            deeps[b] += 1
            _inner_deep_ref(fields[b])
...
def _fn_name_default_factory(name):
    return f'__dataclass_df_f_n_{name}__'


def _fn_name_update(name):
    return f'__dataclass_update_f_n_{name}__'


def _fn_name_setter(name):
    return f'__dataclass_set_f_n_{name}__'


def _fn_name_getter(name):
    return f'__dataclass_get_f_n_{name}__'


def _field_init(f, is_local, frozen, globals, self_name, slots):
    # Return the text of the line in the body of __init__ that will
    # initialize this field.

    default_name = _fn_name_default_factory(f.name)
    ...
    if is_local:
        return _field_assign(frozen, f"_{f.name}", value, self_name)
...
def _init_param(f):
    ...
    if f.default is MISSING and f.default_factory is MISSING:
        ...
        # it up.
        default = f'={_fn_name_default_factory(f.name)}'
...
def _init_fn(fields, ref_fields, std_fields, kw_only_fields, frozen, has_post_init,
    ...
    body_lines = []
    for f in fields:
        line = _field_init(f, f in ref_fields, frozen, locals, self_name, slots)
    ...
    for f in ref_fields:
        if f._field_type == _FIELD_CALC:
            # 初始化调用更新所有值
            default_name = _fn_name_default_factory(f.name)
            args = ','.join([f'{self_name}.{n}' for n in f._ref_fields_names])
            body_lines.append(f'{self_name}._{f.name}={default_name}({args})')
...
def _update_fn(field, globals):
    default_name = _fn_name_default_factory(field.name)
    globals[default_name] = field.default_factory
    args = ','.join([f'self.{n}' for n in field._ref_fields_names])
    body_lines = [f'self.{field.name}={default_name}({args})']
    return _create_fn(field.name,
                      ('self',),
                      body_lines,
                      globals=globals)


def _setter_fn(field, globals):
    default_name = _fn_name_setter(field.name)
    globals[default_name] = field.default
    body_lines = [f'self._{field.name}=value']
    for ff in field._follow_fields or ():
        body_lines.append(f'self.{_fn_name_update(ff.name)}()')
...
def _getter_fn(field, globals):
    default_name = _fn_name_getter(field.name)
    globals[default_name] = field.default
    body_lines = [f'return self._{field.name}']
    return _create_fn(field.name,
                      ('self',),
                      body_lines,
                      globals=globals)
...
def _get_field(cls, a_name, a_type, default_kw_only):
    ...
    if f._field_type is _FIELD:
        ...
        args = (tuple(inspect.signature(f.default_factory).parameters.keys())
                if f.default_factory and callable(f.default_factory) else None)
        if (_is_initvar(a_type, dataclasses)
            ...
            f._field_type = _FIELD_CALC
            f._ref_fields_names = args
    ...
    if f._field_type in (_FIELD, _FIELD_INITVAR):
        ...
        assert f._field_type is _FIELD_CALC
        if f.kw_only is not MISSING:
            raise TypeError(f'field {f.name} is a calculated field but specifies '
                            'kw_only')
...
def _process_class(cls, init, repr, eq, order, unsafe_hash, frozen,
    ...
    for f in fields.values():
        if f._ref_fields_names:
            ls = []
            for f_name in f._ref_fields_names:
                fd = fields[f_name]
                ls.append(fd)
                fd._add_follow_field(f)
            f._ref_fields = ls
    ...
    all_init_fields = [f for f in fields.values()
                       if f._field_type in (_FIELD, _FIELD_INITVAR)]
    all_calc_fields = [f for f in fields.values()
                       if f._field_type == _FIELD_CALC]
    # 检查回环依赖
    _validate_ref_fields(all_calc_fields)
    deeps = _deep_ref(fields)
    having_ref_fields = [fields[k] for k, v in deeps.items() if v > 0 or fields[k]._field_type == _FIELD_CALC]
    # 按深度倒序
    sorted_having_ref_fields = sorted(having_ref_fields, key=lambda f: deeps[f.name], reverse=True)
    ...
    for field in sorted_having_ref_fields:
        if field._field_type == _FIELD_CALC:
            # 创建更新函数 更新自动计算值
            _set_new_attribute(cls, _fn_name_update(field.name), _update_fn(field, globals))
        # 设置特殊属性 设置时调用对应的更新函数
        _set_new_attribute(cls, field.name, property(fget=_getter_fn(field, globals), fset=_setter_fn(field, globals)))

    # Get the fields as a list, and include only real fields.  This is
    # used in all of the following methods.
    field_list = [f for f in fields.values() if f._field_type is _FIELD or _FIELD_CALC]
